...
{
 this.browser='';  this.isOpera=false;  this.isIE=false;  this.isFF=false;  this.isNetscape;  this.isMozilla = false; this.isSafari = false; this.version='';  this.isMacOS=false; this.isSafari2x = false; this.isiPad = false; this.isiOS5 = false; this.isEdge = false; this.isChrome = false;}

PT_browserInfo.prototype = {
init:function(){
this.browser = navigator.userAgent.toLowerCase();this.isMozilla = /mozilla/.test(this.browser) && !/(compatible|webkit)/.test(this.browser);this.isSafari2x = this.Safari2x(this.browser);this.isiPad = (this.browser.indexOf('ipad') >= 0) ? true : (this.browser.indexOf('iphone') >= 0) ? true : false;this.isiOS5 = /os 5/.test(this.browser);this.isOpera = (this.browser.indexOf('opera') >= 0) ? true : false;if (this.isOpera) return;this.isIE = ((this.browser.indexOf('msie') >= 0) || (this.browser.indexOf('trident') >= 0) || (this.browser.indexOf('edge') >= 0)) ? true : false;if (this.isIE) {
...
{
 var elemAbove = document.getElementById(form.name+'divSEARCHABOVE'); var elemAdv = document.getElementById(form.name+'divSEARCHADV'); var elemBelow = document.getElementById(form.name+'divSEARCHBELOW'); var elemImgCollapse = document.getElementById('collapseSrchCriteria'); var elemImgExpand = document.getElementById('expandSrchCriteria'); var elemTblPsTabs = document.getElementById('tblpstabs');  if (bExpand) {
 if (elemAbove) {
...
if (isFModeLayout()) closeLastModal();var bIsBulkAction = false;if (typeof (bBulkAction) != 'undefined' && bBulkAction != null && bBulkAction == 1)
...
PT_Search.prototype = {
 ...
 function() {
 var srchGrpList = this.req.responseText;  if (srchGrpList == "") {
 ...
 var sPortalGSUrl = sPortalUrl + "/c/PORTAL_ADMIN.PTSF_GBLSRCH_FLUID.GBL";  var res = srchGrpList.split("_:_END,"); for (var i = 0; i < (res.length - 1); i++) {
...
function pingRemotenode()
{
 ...
 var baseUri = location.href; var remoteNodeList = pingInfo; var expires = new Date(); expires.setTime(expires.getTime() + (1000 * 24 * 60 * 60 * 1000)); setCookie("psping",remoteNodeList + " STATUS:DONE",expires,'/',document.domain,cookieObject.isSecure()); var remoteNodes = remoteNodeList.split(" "); for (var i = 0; i < remoteNodes.length; i++) {
 var nodeUri = baseUri.split(getPortalName(baseUri))[0]; nodeUri = nodeUri.replace("/psc/", "/psp/"); nodeUri = nodeUri + getPortalName(baseUri) + "/" + remoteNodes[i] + "/s/WEBLIB_PTPP.ISCRIPT1.FieldFormula.IScript_SSOTester?cmd=smartnav"; loader = new net2.ContentLoader(nodeUri,null, null, "GET", function() {
...
PT_Dialog.prototype = {
 ...
 modObj.bPopup = (value.indexOf("0") != -1) ? false : true; break; case "sPopupParentId":
 ...
 modObj.bAutoClose = true; if (isFModeLayout() && isAccessibleLayout() && (modObj.sStyle.indexOf('ps_calendar_modal') == -1) && modObj.bDivPopup) { 
 ...
 this.processOptions(options, modObj, url);  if (modObj.bMsg && isFModeLayout() && !oParentWin.isFModeLayout())
 ...
 role += "aria-labelledby='clndrtitle' "; else
 role += "aria-labelledby='ptModTitle_" + this.idCount + "' "; if (modObj.bFolder) {
 ...
 sHtml += "<div id='ptModContent_" + this.idCount + "' CLASS='ps_modal_content psc_has_iframe'><iframe frameborder=0 id='" + MTop().PTMODFRAME_ + this.idCount + "' name='" + MTop().PTMODFRAME_ + this.idCount + "' title='" + sPopupTitle + "' src='" + url + "' width=" + dWidth + " height=" + dHeight + sScrollingNo + "></iframe></div>"; }
 ...
 if (isFModeLayout() && typeof modObj.oParentWin.gFocusObj != "undefined" && modObj.oParentWin.isFModeLayout() && !modObj.oParentWin.gFocusObj)
 ...
 modObj.style.display = "none"; modObj.style.zIndex = "-1"; modObj.bRemove = true; if (!isFModeLayout()) {
 ...
 if (isFModeLayout()) return; var sScript = "processing_empty"; if (eval ("typeof " + sScript + " !== 'function'")) {
 ...
 if (!modObj.bMsg && !modObj.bCustResizeDone && specModalPage && (w < 350 || h < 350))
 {
 minW = 400; if (h < 350)
 minH = 350;  else 
 minH = h;  }
 ...
 modObj.oParentWin.bProcess = false; this.setModalDialogTitle(id);  if (isFModeLayout() && oframe && isIOS()) {
 ...
 this.setFocusFMode(id); else if (modObj.bPopup && !modObj.bTypeAhead && !modObj.bRCFModal) { 
...
positionPopup: function (modObj, otb, id) {
 if (!isFModeLayout() || !modObj.bPopup || modObj.bFullScreen || modObj.bCenter) return;  var oBody = document.getElementsByTagName("BODY")[0]; oBody.style.overflow = "hidden"; if (browserInfoObj2.isFF || browserInfoObj2.isIE && !browserInfoObj2.isEdge)
 oBody = document.documentElement; var oframe = MTop().document.getElementById(MTop().PTMODFRAME_ + id); if (oframe) {
 ...
 otb.style.left = nParentRight + oBody.scrollLeft + 'px'; }
 else {
 ...
 otb.style.right = nRight - oBody.scrollLeft + 'px'; }
 ...
 nLeft = nParentLeft; if (nLeft < 0) nLeft = nParentRight - otb.clientWidth; if ((nLeft + otb.clientWidth) > fullWidth) nLeft = fullWidth - otb.clientWidth -4; if (nLeft < 10) nLeft = 10; var nRight = fullWidth - (nLeft + otb.clientWidth); otb.style.left = nLeft + oBody.scrollLeft+'px'; if (!modObj.bTypeAhead) otb.style.right = nRight - oBody.scrollLeft + 'px'; if (modObj.bTypeAhead || oframe) {
 ...
 if (fullWidth < 360 && isFModeLayout() && isIOS() && modObj.bCal)bDown = true;  if (bDown) {
...
setModalDialogTitle: function(id, sTitle) {
 ...
 var oTitleBar = MTop().document.getElementById("ptModTitleBar_" + id); if (!oTitleBar) return; var obottom = MTop().document.getElementById("ptModBottom_" + id); var octl = MTop().document.getElementById("ptModControl_" + id); var oresize = MTop().document.getElementById("ptModResize_" + id);  oTitleBar.onmousedown = null; if (octl) octl.style.display = "none"; oTitleBar.style.cursor = "auto"; },
...
function closeLastModal(evt, target, oParentWin) {
 if (!isLastLocalModal() && window.modalID == MTop().modId) return true; if (target && getPTDialog().isInModal(MTop().modId, target)) return true; if (isLastLocalModal())
...
function autoCloseTypeAhead(evt) {
 var id = getPTDialog().getTypeAheadModalId(); if (id.length == 0) return; if (isTypeHeadeNew()) return; getPTDialog().doCloseModalDialog(id); setLocalModal(false);}
...
glObjTr.createMenu = function (sIdEsc, sParentId, nType, sIdIEsc, sIdCh, sNId, nDescr, nLeft, nTop, bIE, o) {
 ...
 MyList.setAttribute('role', "menu"); MyList.setAttribute('aria-labelledby', ariaLabel); var sMyInnerHTML = ""; sMyInnerHTML += "<table class=\'PTCPRCMENUTABLE\' role=\'presentation\' SMN=1 cellspacing=\'0px\' cellpadding=\'0px\' border=0 style=\'border-collapse:collapse;margin-top:8px;margin-bottom:10px; "; sMyInnerHTML += "\'>";  var sVal = ""; var sIdIt = ""; var bCloseWindow = false; var sClassIt = "PT_ACTION_LIST_ITEM"; var nListLen = aDescr.length; for (var f = 0; f < nListLen; f++) {
 ...
 {
 sMyInnerHTML += "<tr MSG=1 onclick=\"glObjTr.preventEventPropagation(event);\" id=" + sTrId + " >";  sMyInnerHTML += "<th class='PTCPRCMENUSEPERATORTH' colspan=2><hr class='PTCPRCMENUSEPERATORHR' color='#BBBBBB'></th>"; sMyInnerHTML += "</tr>"; } 
...
function getSrchRsltRelatedActions() {
 ...
 actionurl = tmpActionUrl.concat('&ICAJAX=1');   var sLoader = new net2.ContentLoader(actionurl, null, null, "POST",
 function() {
...
function DoUrlNUI() {
 sessionStorage.removeItem("chromesameredirectindecator"); if (window != top && isPortalUrl(top.location.href) && !StayInFrame())
 {
 
 
 sessionStorage.setItem("chromesameredirectindecator", window.location.href); top.location.href = window.location.href; }
}
...
addNUIToBC : function () {
 ...
 var eAppBreadCrumb = eAppBreadCrumbContainer.removeChild(eAppBreadCrumbContainer.children[1]); eAppBreadCrumbContainer.parentNode.removeChild(eAppBreadCrumbContainer); AddToHistory(eAppBreadCrumb.children[7].innerHTML, "", "", eAppBreadCrumb.children[6].innerHTML, 0, 0, "", 0, eAppBreadCrumb);   return true; }
...
function UpdateHistory(label, keyData, userData, pageName, valid, userQueryString, nPost, bReturnToLastPage)
 {
 if (!backNavigation.shouldUpdateHistory(pageName)) { return; }
 ...
 var PgltAreaFrame = top.document.getElementById('ptalPgltAreaFrame'); var testURL = top.document.getElementById('ptalPgltAreaFrame').baseURI; if (typeof testURL == "undefined" || !testURL) { testURL = top.document.location.href; }
...
function AddToHistory(label, keyData, userData, pageName, stateNum, elemNum, classicURL, dashboard, appBcData, nPost, userQueryString, bReturnToLastPage) {
 ...
 if (isFModeLayout()) {
 var sameRedirectUrl = sessionStorage.getItem("chromesameredirectindecator"); if(typeof sameRedirectUrl != 'undefined' && sameRedirectUrl && window.location.href == sameRedirectUrl) {
 
 return; }
 }
 sessionStorage.removeItem("chromesameredirectindecator");   if (window.history && window.history.length == 1) { resetHistory(); }

 backNavigation.classicBackButton.create(); if (!backNavigation.shouldAddToHistory(pageName)) { return; }
 ...
 var keyList = ""; if (url.indexOf("?") !== -1) {
 var urlParts = url.split("?"); url = urlParts[0]; var qVars = urlParts[1].split("&"), bFirstKey = true; for (var i = 0; i < qVars.length; i++) {
 qParamNameValue = qVars[i].split("="); if (typeof qParamNameValue[0] != "undefined" && qParamNameValue[0] && qParamNameValue[0].toLowerCase() != "page" && qParamNameValue[0].toLowerCase() != "ptrmtback") { 
 ...
 histRec,
 topRec;  if (pt_history.size() > 0 && typeof top.location.href != 'undefined' && top.location.href && top.location.href.indexOf("ptrmtback=1") > 0) {
 var currTopRec = pt_history.pop(); if (currTopRec.pageName == historyData.pageName && currTopRec.url.split("?")[0] == historyData.url.split("?")[0]) {
 pt_history.save(); } else {
 pt_history.push(currTopRec); }
 }

 
 if (typeof appBcData != 'undefined' && appBcData) {
 ...
 if (!isFModeLayout() && historyData.pageName == '(search)' && !isWorkCenter()) {
 historyData.valid = 0; }
 ...
 var testURL = top.document.getElementById('ptalPgltAreaFrame').baseURI; if (typeof testURL == "undefined" || !testURL) { testURL = top.document.location.href; }
 if (testRec.url == testURL) { historyData.valid = 0; }
 pt_history.push(testRec); }
 }

 
 pt_history = backNavigation.addNonPiaBackRec(pt_history);  if (!backNavigation.isInHistory(historyData)) {
 ...
 var isFromBC = pt_history.isFromBC(); histRec = pt_history.lastValid(pageName, url, historyData); FixupBackToClassicSearch(historyData, pt_history); topRec = pt_history.pop(); if (topRec) {
 ...
 if (typeof bICList == "undefined" || !bICList) { pt_history.push(historyData); }
 }
 }
...
function DoBackClassic() {
 try {
 var eTgtFrame = top.document.getElementById("ptifrmtgtframe"); if (typeof eTgtFrame != "undefined" && eTgtFrame) {
 var eDoBackHandler = eTgtFrame.contentDocument.getElementById("ancdobackclassic"); if (typeof eDoBackHandler != "undefined" && eDoBackHandler) { eDoBackHandler.click(); }
 }
...
function DoBackService(bForm) {
 ...
 if (backRec.url.split("?")[0] == history.nodes[history.size() - 1].url.split("?")[0] && history.nodes[history.size() - 1].pageName.toLowerCase() != "(search)") { 
 history.pop();  } else {
 bDelete = false;  }
 }
 ...
 if (backNavigation.isRemotePiaContent(backRec.url, thisRec.url) && !backNavigation.isIESameOrigin(backRec.url, thisRec.url)) {
 sUrl.indexOf("?") > 0 ? sUrl += "&" : sUrl += "?"; sUrl += "ptrmtback=1"
 }
 ...
 history.pop(); }
 history.save();  if ((backRec && backRec.isWorkCenter) || (thisRec && thisRec.isWorkCenter)) { 
 top.location = sUrl; } else if (typeof backRec != 'undefined' && backRec && backRec.isClassic) {
 top.location = sUrl; } else if (typeof backRec != 'undefined' && backRec && typeof backRec.isClassic != 'undefined' && 
 ...
 window.parent.location.href = sUrl; } else {
 window.location.href = sUrl; }
 } else { 
...
function DoURLWarning(url, histElement, backRec)
{
 ...
 var sJS = 'javascript:window.location.href = "' + url+'"'; psConfirmSW("", sJS, window); }
...
var backNavigation = {
 ...
 var appLabel = document.querySelector("#app_label"), modLabel = top.document.querySelector("#ptModTitle_1"), label = "LabelNotFound"; if (typeof PIA_KEYSTRUCT !== "undefined" && PIA_KEYSTRUCT && typeof appLabel !== 'undefned' && appLabel && appLabel.innerHTML.length >= 1 && /\S/.test(appLabel.innerHTML)) {
 ...
 var referring_site_history = getHistoryObjectFromStr(event.data), pt_history = getHistoryObject(), pt_history_new = null, backBtn = top.document.querySelector("#PT_WORK_PT_BUTTON_BACK"); if (!backBtn) { 
 if (isFModeLayout()) {
 ...
 try {
 if (typeof refUrl == "undefined" || !refUrl || typeof curUrl == "undefined" || !curUrl) return false;  var refUrlNode = getNodeName(refUrl), bNodeFound = false; for (var i=0; i<localNodes.length; i++) {
 ...
 if (typeof winName != 'undefined' && winName && isModalPage(winName) && (typeof bCloseModal != "undefined" && !bCloseModal)) { return false; }

 
 if (typeof bCloseModal != 'undefined' && bCloseModal) {return false;} 
 ...
 var szType = "Back to"; if (typeof bGuidedAG != "undefined" && bGuidedAG) { szType = "Exit to"; }
 ...
 { inObj.setAttribute("aria-label", szType +" "+ szLabel); }
 },
 ...
 var szLabel = this.genLabel(inLabel, histRec, histData); backTextObj.innerHTML = szLabel;  },
 ...
 var eDropDownMenu = top.document.querySelector("#ptdropdownmenu"); if (eDropDownMenu && getComputedStyle(eDropDownMenu).display == "none") 
  ...
  var isHomepage = /\/h\/\?tab=DEFAULT/.test(location);  if(!isHomepage)
  {
 ...
 typeof bHideClassicBackBtn != "undefined" && bHideClassicBackBtn ? this.hide(el) : el.style.visibility = "";  },
 ...
 var dropDownCntnr = top.document.querySelector("#pthdr2container");  var divEl00 = document.createElement("div"); divEl00.setAttribute("id", "classicbackbuttonlayout"); var divEl01 = document.createElement("div"); divEl01.setAttribute("class", "ps_box-button ps_button_backnav psc_header-all"); divEl01.setAttribute("id", "classicbackbuttoncontainer"); var spanEl = document.createElement("span"); spanEl.setAttribute("class", "ps-button-wrapper"); spanEl.setAttribute("title", "Back"); var ancEl = document.createElement("a"); ancEl.setAttribute("id", "PT_WORK_PT_BUTTON_BACK"); ancEl.setAttribute("class", "ps-button"); ancEl.setAttribute("role", "button"); var href = "javascript:DoBackClassic()"; ancEl.setAttribute("href", href); var imgEl = document.createElement("img"); imgEl.setAttribute("src", "https://cmsweb.cs.csueastbay.edu/cs/CEBPRD/cache_85608/PT_NUI_BACK_PRIM_IMG_1.svg"); imgEl.setAttribute("name", "PT_WORK_PT_BUTTON_BACK$IMG"); imgEl.setAttribute("class", "ps-img"); backNavigation.setAltText(imgEl); var spanTxtEl = document.createElement("span"); spanTxtEl.setAttribute("class", "ps-text"); ancEl.appendChild(imgEl); ancEl.appendChild(spanTxtEl); spanEl.appendChild(ancEl); divEl01.appendChild(spanEl); divEl00.appendChild(divEl01); dropDownCntnr.insertBefore(divEl00, dropDownCntnr.children[0]);  }
...
var remoteDataGetter = {
 ...
 if ( referrer.includes("/c/") )
 referrer = referrer.split("/c/")[0]; else if( referrer.includes("/h/") )
 referrer = referrer.split("/h/")[0]; if (backNavigation.isRemotePiaContent(referrer, contentUrl) && !backNavigation.isIESameOrigin(referrer, contentUrl)) { 
 
 var eRemoteDataGetter = document.createElement("iframe"); var iframeId = this.iframeId; if (typeof idAdd != 'undefined' && idAdd) { iframeId += "idAdd" }
 eRemoteDataGetter.setAttribute("name", iframeId);  eRemoteDataGetter.setAttribute("id", iframeId); eRemoteDataGetter.setAttribute("style", "display:none"); document.body.appendChild(eRemoteDataGetter);   eRemoteDataGetterUrl = referrer + "/ptgetremotedata" + "?d=" + document.domain + "&postFunction=" + postFunctionId; eRemoteDataGetter.setAttribute("src", eRemoteDataGetterUrl);  if (typeof msgEventHandler != 'undefined' && msgEventHandler) { window.top.addEventListener("message", msgEventHandler, false); }
 } 
 } catch(e) {}
 },
 ...
 var iframeId = this.iframeId; if (typeof idAdd != 'undefined' && idAdd) { iframeId += "idAdd"; }
